"""
Pika AIGC adapter.
Uses Pika's REST API for text-to-video generation.
"""
import asyncio
import aiohttp
...
class PikaAdapter(AIGCAdapter):
    name = "pika"

    def __init__(self, api_key: str = "", base_url: str = "https://api.pika.art", download_dir: str = "./downloads"):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=body, headers=headers) as resp:
                if resp.status != 200:
                    ...
                    raise Exception(f"Pika API error ({resp.status}): {text}")
                data = await resp.json()
                task_id = data.get("id", data.get("job_id", "unknown"))
    ...
    async def query_result(self, handle: TaskHandle) -> TaskResult:
        url = f"{self.base_url}/v1/generate/{handle.submit_id}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                ...
                if status in ("succeeded", "complete"):
                    video_url = data.get("video_url", data.get("output", {}).get("video_url", ""))
                    return TaskResult(status="success", file_path=video_url, raw_response=data)
                elif status in ("failed", "error"):
                    return TaskResult(status="failed", error=data.get("error", "Unknown error"), raw_response=data)
                else:
    ...
    async def _download_file(self, url: str, dest_dir: str) -> str:
        ...
        local_path = Path(dest_dir) / f"pika-{url_hash}.mp4"
        async with aiohttp.ClientSession() as session:
